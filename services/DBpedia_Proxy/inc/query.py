...
async def runQuerySingleKey(cache, entities, stmt, max_entity=20, printIt=False):
    """
    ...
    for chunk in chunks:
        ...
        valuelist = ' '.join(['<{}>'.format(el['base']) for el in chunk])
    ...
    for i in range(len(resp)):
        ...
        # the first element in the part is an empty dictionary
        # elif 'base' not in part:
            # keep track of the entities, we dont have an answer for (not so sure of the solution)
            # entities_noResponse.update(chunks[i])
        else:
            raw.extend(part)
...
async def runQueryMultiKey(cache, values, stmt, max_entity=None):
    ...
    if max_entity:
        ...
        for base in bases:
            ...
            # format base entities
            formated_values['base'] = ' '.join([f"<{el}>" for el in base])
