...
async def runQuerySingleKey(cache, entities, stmt, max_entity=10000):
    """
    ...
    cached = cache.getMany([{'base': e} for e in entities])
    ...
    # store into cache
    if missed:
        cache.setMany([{'key': {'base': entity}, 'val': []} for entity in missed])
    if aggregated:
        cache.setMany(aggregated)
...
async def runQueryMultiKey(cache, values, stmt, max_entity=None):
    ...
    if missed:
        ...
        cache.setMany([{'key': entry, 'val': []} for entry in entries])
    if aggregated:
        cache.setMany(aggregated)
