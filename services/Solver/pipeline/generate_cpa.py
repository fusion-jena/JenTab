...
def generate(pTable, proxyService, skip_obj=False):
    """
    ...
    if skip_obj:
        ...
        subjLookup = proxyService.get_properties_for_lst.send([subjCands, 'en'])
    else:
        subjLookup = proxyService.get_properties_for_lst.send([subjCands, 'en'])
...
def __match_fuzzy_string(kg_val, val):
    util_log.debug("String Value compared: KG Val =  {} Table Val: {}".format(kg_val, val))

    kg_val = remove_special_chars(kg_val).lower()

    val_words = parse_string(val)
    util_log.debug("After Cleaning: KG Val = {} Table Val: {}".format(kg_val, ' '.join(val_words)))

    w_cnt = 0
    for w in val_words:
        w = remove_special_chars(w).lower()
        util_log.debug("Found one word: {}".format(w))
        if kg_val.find(w) > -1:
            w_cnt = w_cnt + 1

    util_log.debug("Matched {} words".format(w_cnt))
...
def __match_fuzzy_numbers(kg_val, val):
    util_log.debug("Number Props compared: kg_val: {} val: {}".format(kg_val, val))
    try:
        kg_val = float(kg_val)
        val = float(val)
...
def __match_dates(kg_val, val):
    util_log.debug("DATE Value compared: KG Val =  {} Table Val: {}".format(kg_val, val))
    try:
        d1 = parser.parse(kg_val)
        d2 = parser.parse(val)
        util_log.debug("Parsed Vals: KG_Val  {} Table Val: {}".format(d1, d2))
        # Match if two dates share the same day, month and year
        if d1.day == d2.day and d1.month == d2.month and d1.year == d2.year:
            return True
    except:
        util_log.debug(f"Failed to parse as date: '{kg_val}' or '{val}'" )
        return False
