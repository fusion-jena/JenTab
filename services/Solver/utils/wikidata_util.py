...
def get_wiki_type(type):
    ...
    """
    try:
        return WIKITYPE_MAPPING[(type or '').upper()]
    except Exception: # in case of a predicted type not in supported mappings, handle as a string.
        # util_log.error( f"Wikidata-Type: unknown type \"{type}\"")
        return wikiString
