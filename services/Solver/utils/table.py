...
class ParsedTable():
    ...
    def removeCells(self, cells):
        [self._cells.remove(c) for c in cells]
    ...
    def purgeCellPairs(self, purged):
        ...
        # for speedup: group by row_id and col_id, so we have easier lookups later on
        lookup = {}
        for item in purged:
            if item['row_id'] not in lookup:
                lookup[ item['row_id'] ] = {}
            if item['col_id'] not in lookup[ item['row_id'] ]:
                lookup[ item['row_id'] ][ item['col_id'] ] = set()
            lookup[ item['row_id'] ][ item['col_id'] ].add( item['uri'] )
        ...
        self._cellpairs = [
            ...
            if
                # there is no entry for the whole row
                (pair['row_id'] not in lookup)
                or (
                      # not purged because of the subject
                      (
                           (pair['subj_id'] not in lookup[ pair['row_id'] ])
                        or (pair['subj_cand'] not in lookup[ pair['row_id'] ][ pair['subj_id'] ])
                      )
                  and # not purged because of the object
                      (
                           (pair['obj_id'] not in lookup[ pair['row_id'] ])
                        or (pair['obj_cand'] not in lookup[ pair['row_id'] ][ pair['obj_id'] ])
                      )
                )
        ]

    def setCellPairs(self, pairs):
        """
        replace the current list of cell pairs
        """
        self._cellpairs = pairs
    ...
    def printCEA(self, row_id=None, col_id=None):
        ...
        if col_id is not None and not isinstance(col_id, list) and not isinstance(col_id, range):
            col_id = [col_id]
        if row_id is not None and not isinstance(row_id, list) and not isinstance(row_id, range):
            row_id = [row_id]

        # filter down to requested values
        cells = [cell for cell in self._cells if cell['type'] == config.OBJ_COL]
        if row_id is not None:
            cells = [cell for cell in cells if cell['row_id'] in row_id]
        if col_id is not None:
            cells = [cell for cell in cells if cell['col_id'] in col_id]
    ...
    def printCPA_rows(self, row_id=None, subj_cand=None, obj_id=None):
        ...
        if row_id is not None and not isinstance(row_id, list) and not isinstance(row_id, range):
            row_id = [row_id]
        if subj_cand is not None and not isinstance(subj_cand, list) and not isinstance(subj_cand, range):
            subj_cand = [subj_cand]
        if obj_id is not None and not isinstance(obj_id, list) and not isinstance(obj_id, range):
            obj_id = [obj_id]

        # filter down to requested values
        pairs = self._cellpairs
        if row_id is not None:
            pairs = [pair for pair in pairs if pair['row_id'] in row_id]
        if subj_cand is not None:
            pairs = [pair for pair in pairs if pair['subj_cand'] in subj_cand]
        if subj_cand is not None:
            pairs = [pair for pair in pairs if pair['obj_id'] in obj_id]
        ...
        data = sorted(data)
        print( tabulate(data, headers=['row_id', 'subj_id', 'subj_cand', 'obj_id', 'obj_cand', 'cand'], tablefmt='orgtbl'))


    def printTable(self, lines=-1):
        """
        pretty print the table as a whole
        cells with candidates are marked green, those without are red
        """
        data = []
        for row_id in range(0, lines if (lines > 0) and (lines < self.rowcount) else self.rowcount):
